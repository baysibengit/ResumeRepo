...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string GenerateUniqueUid() {
            ...
            // Extract all uIDs
            var adminIds = db.Administrators.Select(a => a.Id);
            var studentIds = db.Students.Select(s => s.Id);
            var professorIds = db.Professors.Select(p => p.Id);
            ...
            // Generate randomized uIDs until unique uID is encountered
            do
            {
                uniqueNum = random.Next(1000000, 9999999);
                uID = "u" + uniqueNum.ToString();
            }
            while (allIds.Contains(uID)); 
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            // Add user to the appropriate table based on the role
            switch (role)
            {
                case "Administrator":
                    db.Administrators.Add(new Administrator
                    {
                        ...
                        First = firstName,
                        Last = lastName,
                        Dob = new DateOnly(DOB.Year, DOB.Month, DOB.Day)
                    });
                    break;
                case "Professor":
